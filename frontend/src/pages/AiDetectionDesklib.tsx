...
export default function AiDetectionDesklib() {
  ...
  const fetchAiData = (buckets: string[], pg: number, sort: string) => {
    setLoading(true);
    api.aiDetection(buckets, pg, sort, "desklib")
      .then((res) => {
        setData(res);
        setLoading(false);
      })
      .catch((err) => {
        console.error(err);
        setError("Impossibile caricare le analisi di rilevamento IA Desklib.");
        setLoading(false);
      });
  };
  ...
  return (
    <Box>
      <Box sx={{ mb: 6 }}>
        ...
        >
          Rilevamento IA (Desklib)
        </Typography>
        <Typography variant="body1" sx={{ color: "#75758a" }}>
          Classificazione del testo basata su <strong>Desklib AI Detector (desklib/ai-text-detector-v1.01)</strong>, un modello di classificazione supervisionata fine-tuned specializzato nel rilevamento di contenuti sintetici in lingua inglese.
        </Typography>
      </Box>
          ...
          <Grid item xs={12} md={8}>
            ...
            >
              <Typography variant="h6" sx={{ fontWeight: 600, color: "#17171c", mb: 3 }}>
                Spettro di Probabilità Desklib
              </Typography>
                      ...
                      <LinearProgress
                        ...
                        sx={{
                          ...
                          "& .MuiLinearProgress-bar": {
                            backgroundColor: isHighRisk ? "#ff7759" : "#ff7759",
                          },
                        }}
                      />
                    </Box>
        ...
        <Box sx={{ mb: 6 }}>
          <Box sx={{ mb: 3 }}>
            <Typography variant="h5" sx={{ fontFamily: "Space Grotesk", fontWeight: 700, color: "#17171c" }}>
              Campionamento Rappresentativo per Fascia di Probabilità Desklib
            </Typography>
            <Typography variant="body2" sx={{ color: "#75758a", mt: 0.5 }}>
              Campioni estratti da <code>risultati.jsonl</code> per ciascuna fascia di confidenza.
            </Typography>
          </Box>
                          ...
                          <Box key={post.id}>
                            <ListItem disablePadding sx={{ py: 1, px: 0, flexDirection: "column", alignItems: "flex-start" }}>
                              <Box sx={{ display: "flex", justifyContent: "space-between", width: "100%", mb: 0.5 }}>
                                ...
                                <Typography variant="caption" sx={{ fontWeight: 700, color: "#ff7759" }}>
      ...
      <Paper variant="outlined" sx={{ borderRadius: "16px", p: 3, mb: 4, borderColor: "#e5e7eb" }}>
        <Typography variant="h6" sx={{ fontWeight: 600, color: "#17171c", mb: 2 }}>
          Filtra & Esplora i Post Valutati da Desklib
        </Typography>
            ...
            <FormControlLabel
              ...
              control={
                <Checkbox
                  ...
                  sx={{ color: "#75758a", "&.Mui-checked": { color: "#ff7759" } }}
                />
              }
              label={`Fascia ${b}%`}
            />
